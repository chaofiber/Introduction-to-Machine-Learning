...
def svr(X,y):
    model =SVR(kernel='linear', gamma=0.1)
    model.fit(X,y)
    return model
...
def cross_validation_reg(data, Y_train, kfold):
    ...
    # print(score,len(Y_train),len(y_val))
...
def main():
    ...
    # train,test,label = data_processnorm(train_path, test_path, label_path)
