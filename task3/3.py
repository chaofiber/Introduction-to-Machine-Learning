...
def regression(X,y):
    xg_reg = XGBRegressor(objective ='reg:squarederror', colsample_bytree = 0.3, learning_rate = 0.4,
...
def svc(X,y):
	...
	#clf = SGDClassifier();
    #clf = MLPClassifier(solver='sgd', alpha=1e-5,hidden_layer_sizes=(100,100), random_state=1,max_iter = 1000);   
...
def xgb(X,y):
	xg_reg = XGBRegressor(objective ='binary:hinge', colsample_bytree = 0.3, learning_rate = 0.05,
	max_depth = 8, alpha = 10, n_estimators = 2500,scale_pos_weight = 50);
	xg_reg.fit(X,y);
	return xg_reg;
